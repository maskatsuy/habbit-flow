...
const HabitNode = memo(({ data, selected, id }: HabitNodeProps) => {
  const { label, icon, isCompleted, isDisabled, isInactive, isFlowing, onComplete } = data as HabitNodeData & { onComplete?: (nodeId: string) => void };
  ...
  const ringColor = selected && !isDisabled ? 'ring-blue-500' : '';

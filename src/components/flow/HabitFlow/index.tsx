...
import { addEdge, useNodesState, useEdgesState, ReactFlowProvider } from 'reactflow'
import type { Connection } from 'reactflow'
import '../../nodes' // Import for side effects (component registration)
...
function HabitFlowInner() {
  ...
  const [insertingEdge, setInsertingEdge] = useState<{
    edgeId: string
  ...
  const handleInsertNodeCreate = useCallback(
    (nodeData: { label: string; description?: string; icon?: string; timing?: string }) => {
  ...
  const handleNodeEdit = useCallback(
    (nodeId: string) => {
      setNodes((currentNodes) => {
        const node = currentNodes.find((n) => n.id === nodeId && n.type === 'habit') as
          | HabitNodeType
          | undefined
        if (node) {
          setEditingNode(node)
  ...
  // 削除可否をチェックする関数
  const checkNodeDeletability = useCallback(
    (nodeId: string): { canDelete: boolean; reason?: string } => {
      ...
      if (isMergePoint) {
        return {
          canDelete: false,
          reason: 'マージポイント（複数の入力を持つノード）は削除できません',
        }
      }
          ...
          for (const edge of incomingEdges) {
            ...
            if (sourceNode?.type === 'conditional') {
              return {
                ...
                handle: edge.sourceHandle || undefined,
              }
            }
            // さらに上流を探索
    ...
    (changes: Parameters<typeof onNodesChange>[0]) => {
      ...
      if (selectionChanges.length > 0) {
        ...
        if (selectedChange) {
          ...
          const anySelected = nodes.some(
            (n) => !selectionChanges.some((c) => c.id === n.id) && n.selected,
          )
          if (!anySelected) {
  ...
  const handleNodeDelete = useCallback(
    (nodeId: string) => {
      ...
      if (
        !isMergePoint &&
        pathInfo.isConditionalPath &&
        pathInfo.conditionalId &&
        pathInfo.handle
      ) {
        // 同じ条件分岐の同じハンドルから始まるパス上のhabitノードを数える
        const countHabitNodesInConditionalPath = (): number => {
    ...
    <NodeEditorProvider onEditNode={handleNodeEdit} onInsertNode={handleInsertNode}>
      <div className="w-full">
        ...
        <InsertNodeModal
          ...
          sourceNodeLabel={
            insertingEdge
              ? nodes.find((n) => n.id === edges.find((e) => e.id === insertingEdge.edgeId)?.source)
                  ?.data.label
              : undefined
          }
          targetNodeLabel={
            insertingEdge
              ? nodes.find((n) => n.id === edges.find((e) => e.id === insertingEdge.edgeId)?.target)
                  ?.data.label
              : undefined
          }
        />

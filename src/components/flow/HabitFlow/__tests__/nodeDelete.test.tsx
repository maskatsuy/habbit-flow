...
describe('Node Deletion Logic', () => {
  ...
  const testNodeDeletion = (
    ...
    if (!isMergePoint && pathInfo.isConditionalPath && pathInfo.conditionalId && pathInfo.handle) {
      ...
      if (habitNodeCount <= 1) {
        return {
          canDelete: false,
          alertMessage: '条件分岐の各パスには最低1つのノードが必要です。このノードは削除できません。'
        };
      }
    }
  ...
  describe('条件分岐でのノード削除', () => {
    it('条件分岐パスの最後のノードは削除できない', () => {
      ...
      expect(result.alertMessage).toBe('条件分岐の各パスには最低1つのノードが必要です。このノードは削除できません。');
    });
    ...
    it('条件分岐からのエッジはsourceHandleを保持する', () => {
      ...
      const newEdge = result.updatedEdges?.find(e => 
        e.source === 'weather1' && e.target === 'stretch1'
      );

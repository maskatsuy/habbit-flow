...
describe('FlowManager', () => {
  it('should display list of saved flows', () => {
    render(<FlowManager currentFlow="朝のルーティン" onLoadFlow={() => {}} onNewFlow={() => {}} />)
  ...
  it('should highlight current flow', () => {
    ...
    const flowCards = screen.getAllByText(/ノード/).map((el) => el.closest('.p-3'))
    const currentFlowCard = flowCards.find((card) => card?.textContent?.includes('朝のルーティン'))
  ...
  it('should call onLoadFlow when flow is clicked', async () => {
    ...
    render(
      <FlowManager currentFlow="朝のルーティン" onLoadFlow={onLoadFlow} onNewFlow={() => {}} />,
    )
  ...
  it('should call onNewFlow when new flow button is clicked', async () => {
    ...
    render(<FlowManager currentFlow="朝のルーティン" onLoadFlow={() => {}} onNewFlow={onNewFlow} />)
  ...
  it('should delete flow when delete button is clicked', async () => {
    ...
    expect(window.confirm).toHaveBeenCalledWith('フロー「夜のルーティン」を削除しますか？')

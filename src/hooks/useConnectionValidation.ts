...
export function useConnectionValidation(nodes: FlowNode[], edges: FlowEdge[]) {
  ...
  const validateConnection = (connection: Connection): ValidationResult => {
    const { source, target, sourceHandle: _sourceHandle, targetHandle: _targetHandle } = connection
    ...
    const existingConnection = edges.find((e) => e.source === source && e.target === target)

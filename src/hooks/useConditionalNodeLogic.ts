  ...
  const handleNodeClick = useCallback(
    (_: React.MouseEvent, node: FlowNode) => {
      if (node.type === 'habit') {
        setNodes((nds) => {
          // 条件分岐後のノードかチェック
          const isAfterConditional = edges.some(
            (e) =>
              e.target === node.id && nds.find((nd) => nd.id === e.source)?.type === 'conditional',
          )
          ...
          return nds.map((n): FlowNode => {
            ...
            if (isAfterConditional) {
              // クリックされたノードの元の状態を取得
              const clickedNode = nds.find(
                (nd) => nd.id === node.id && nd.type === 'habit',
              ) as HabitNodeType

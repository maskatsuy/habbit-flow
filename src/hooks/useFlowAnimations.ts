      ...
      // ターゲットが習慣ノードの場合
        ...
        if (sourceNode?.type === 'conditional') {
          ...
          if (incomingEdges.length > 1) {
            const isSourceCompleted =
              sourceNode?.type === 'habit' ? (sourceNode as HabitNodeType).data.isCompleted : false
    ...
    const animatedNodes = nodes.map((node) => {
      ...
      if (node.type === 'trigger') {
        ...
        const isNextCompleted =
          nextNode?.type === 'habit' ? (nextNode as HabitNodeType).data.isCompleted : false
      ...
      if (node.type === 'conditional') {
        ...
        const isPrevCompleted =
          prevNode?.type === 'habit' ? (prevNode as HabitNodeType).data.isCompleted : false
      ...
      if (node.type === 'habit') {
        ...
        const findConditionalPath = (
          nodeId: string,
        ): { conditionalId: string; handle: string } | null => {
        ...
        if (conditionalPath && !isMergePoint) {
          ...
          // 自分のパスに完了ノードがあるかチェック
          const hasCompletedNodeInMyPath = myPathEdge
            ? checkPathForCompletedNodes(myPathEdge)
            : false

          // 他のパスに完了ノードがあるかチェック
          const hasCompletedNodeInOtherPath = otherPathEdges.some((edge) =>
            checkPathForCompletedNodes(edge),
          )
        ...
        return {
          ...
          data: {
            ...habitNode.data,
            isInactive,
            isFlowing: isTerminalNode
              ? habitNode.data.isCompleted
              : incomingActive && outgoingActive,
          },
        }
      }
    ...
    const processedEdges = edges.map((edge) => {
      ...
      if (targetNode?.type === 'habit') {
        ...
        if (incomingEdges.length > 1) {
          ...
          return {
            ...edge,
            data: { ...edge.data, isActive: isSourceCompleted && isTargetCompleted },
          } as FlowEdge
        }

...
describe('useFlowAnimations - マージポイントの状態管理', () => {
  ...
  it('マージポイントは条件分岐の影響を受けない', () => {
    ...
    console.log('マージポイントテスト結果:', {
      ...
      jog: { id: jogNode?.id, isInactive: jogNode?.data.isInactive, isCompleted: jogNode?.data.isCompleted },
      bike: { id: bikeNode?.id, isInactive: bikeNode?.data.isInactive, isCompleted: bikeNode?.data.isCompleted },
      shower: { id: showerNode?.id, isInactive: showerNode?.data.isInactive, isCompleted: showerNode?.data.isCompleted },
    });
